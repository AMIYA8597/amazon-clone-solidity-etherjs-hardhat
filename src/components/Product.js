...
const Product = ({ item, provider, account, amazon, togglePop }) => {
  ...
  const fetchDetails = async () => {
    ...
    const order = await amazon.orders(account, orders[0].args.orderId)
    setOrder(order)
  }
  ...
  const buyHandler = async () => {
    ...
    // Buy item...
    let transaction = await amazon.connect(signer).buy(item.id, { value: item.cost })
    ...
    <div className="product">
      <div className="product__details">
        ...
        <div className="product__order">
          ...
          <p><small>Ships from</small> Amazon</p>
          <p><small>Sold by</small> Amazon</p>
